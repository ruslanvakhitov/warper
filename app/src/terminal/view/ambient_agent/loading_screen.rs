//! Loading screen UI for removed hosted agent initialization paths.
...
/// Renders the removed hosted agent loading screen with shimmering warp logo and tips.
...
/// Renders the removed hosted agent error screen.
...
/// Renders the removed hosted agent cancelled screen.
pub fn render_cloud_mode_cancelled_screen(appearance: &Appearance) -> Box<dyn Element> {
    ...
    let subtitle_text = Text::new(
        "No hosted environment was started",
        appearance.ui_font_family(),
        appearance.monospace_font_size(),
    )
