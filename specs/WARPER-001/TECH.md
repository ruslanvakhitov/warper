...
- Product invariant 20: include the denylist and offline smoke in the presubmit/toolchain path so regressions are caught without private credentials or hosted service availability. The local smoke command is `./script/warper_offline_local_smoke`; it runs `./script/check_warper_static_denylist` for `startup`, `startup-network`, `entrypoints`, `hosted-restore-startup`, `telemetry-crash-update`, and `all-runtime`, focused `warp_core` channel tests, `cargo check -p warp --bin warp-oss`, the `warp-oss --help` network-intercept smoke, and `./script/warper_terminal_session_network_smoke`.
  The terminal-session smoke builds `warp-oss` and runs its hidden `--warper-local-terminal-smoke` entrypoint under `./script/warper_network_intercept_smoke`; this uses the same early terminal-server/PTY spawn path as GUI startup, requests a local terminal session, stops it, and fails on nonlocal DNS/connect attempts.
  The local smoke must print the exact validations it performs and the explicit gaps it does not cover: no macOS `.app` launch, no OS-level outbound network block, no full `.app` GUI terminal-session network interception command, no manual live UI terminal/pane/settings exercise, no app crate `#[cfg(test)]` hosted-feature modules, no private credentials, no hosted Warp/Oz availability, and no OpenRouter behavior validation.
